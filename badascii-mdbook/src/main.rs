...
pub fn make_app() -> Command {
    Command::new("badascii-mdbook")
        .about("A mdbook preprocessor which renders badascii blocks into SVGs")
        .subcommand(
...
fn main() {
    ...
    if let Some(sub_args) = matches.subcommand_matches("supports") {
        handle_supports(&BadAscii, sub_args);
    } else if let Err(e) = handle_preprocessing(&BadAscii) {
        eprintln!("{e}");
        process::exit(1);
    }
}
